...
class ConnectFourBoard(object):
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(self._k, new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id())
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._k, self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id())
...
class ConnectLongestBoard(object):
    ...
    def do_move(self, column):
        ...
        return ConnectLongestBoard((self._k-1), new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id())
    ...
    def is_win(self):
        ...
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
                cell_player = self.get_cell(i,j)
                if cell_player != 0:
                    tmp = self._max_length_from_cell(i,j)
                    if tmp > max:
                        win = cell_player
                        max = tmp
                        tie = False
                    if cell_player != win and max == tmp:
                        tie == True
        if tie:
            return 0
        else:
    ...
    def clone(self):
        """ Return a duplicate of this board object """
        return ConnectLongestBoard(self._k, self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id())
    ...
    def run_game(self, verbose=True):
        ...
        while not win_for_player and not self._board.is_tie():
            for callback, id, symbol in ( player1, player2 ):
                ...
                while not has_moved:
                    try:
                        new_column,ret_exp,d_time = callback(self._board.clone())
                        if id == 1:
                            player1_expanded = player1_expanded + ret_exp
                            player1_time = player1_time + d_time
                        else:
                            player2_expanded = player2_expanded + ret_exp
                            player2_time = player2_time + d_time
                ...
                if self._board.is_game_over():
                    ...
                    break
        print ""
        print "player1 Node expanded:" +str(player1_expanded)
        print "player2 Node expanded:" +str(player2_expanded)
        print "player1 Running time:" +str(player1_time)
        print "player2 Running time:" +str(player2_time)
        print ""
        win_for_player = self._board.is_win()
...
class ConnectLongestRunner(object):
    """ Runs a game of Connect Four.
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def __init__(self, player1_callback, player2_callback, board = ConnectLongestBoard(10), time_limit = 10):
        """ Create a new ConnectFourRunner.
...
def run_longest_streak(player1, player2, board = ConnectLongestBoard(20)):
    """ Run a game of Connect Four, with the two specified players """
    game = ConnectLongestRunner(player1, player2, board=board)
    return game.run_game()
