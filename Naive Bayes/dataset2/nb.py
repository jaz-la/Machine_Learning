...
for number in range(Class_num):
    d = list(range(len(number_idx[number]))) # number of images which represents number
    count =list(range(height*width)) # number of pixels
    for pixel in range(height*width):
        for i,idx in enumerate(number_idx[number]):
            tmp = Imagedata[idx][pixel]
            d[i] = tmp # d[i] = [1,0,1,0,1,0....]
        count[pixel] = collections.Counter(d) # count[pixel] = {0:10,1:10,2:10}
    ...
    feature_map[number] = count # count = [ [0.3,0.4,0.3], [0.4,0.4,0.2],...28*28... ]

############################################################
#read test image and features
testImage = open('images', 'r')
